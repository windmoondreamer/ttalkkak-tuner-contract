...
> **2026-08-27 — 채워졌다.** Windows 러너에서 C# 출력이 골든과 byte-for-byte 같음을 확인했다.
> 첫 실행은 두 번 실패했는데 계약 불일치가 아니라 Windows 의 Python 콘솔 인코딩(cp1252) 문제였다.
> 그 두 건도 스크립트에서 고쳐 두었으므로, Windows 에서 직접 돌려도 같은 결과가 나온다.
...
| 3 | Swift 구현이 골든과 byte-for-byte 같은지 | ✅ CI 확인 (2026-08-27) |
| 4 | C# 구현이 골든과 byte-for-byte 같은지 | ✅ CI 확인 (2026-08-27) |
